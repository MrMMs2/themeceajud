...
class theme_ceajud_core_renderer extends core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = theme_ceajud_performance_output($perf);
            }
        }
    ...
    public function navigation_menu() {
        ...
        if (isloggedin() && !isguestuser()) {
            ...
            if (!empty($PAGE->theme->settings->enableevents)) {
                $branchtitle = get_string('events', 'theme_ceajud');
            ...
            if (!empty($PAGE->theme->settings->enablemysites)) {
                $branchtitle = get_string('mysites', 'theme_ceajud');
                ...
                if ($sortedcourses) {
                    ...
                    $noenrolments = get_string('noenrolments', 'theme_ceajud');
            ...
            if (!empty($PAGE->theme->settings->enablethiscourse)) {
                if (ISSET($COURSE->id) && $COURSE->id > 1) {
                    $branchtitle = get_string('thiscourse', 'theme_ceajud');
                    ...
                    $data = theme_ceajud_get_course_activities();
    ...
    public function tools_menu() {
        ...
        if (!empty($PAGE->theme->settings->toolsmenu)) {
            $custommenuitems .= "<i class='fa fa-wrench'> </i>".get_string('toolsmenulabel', 'theme_ceajud')."|#|".
                    get_string('toolsmenulabel', 'theme_ceajud')."\n";
    ...
    public function ceajudblocks($region, $classes = array(), $tag = 'aside') {
...
class theme_ceajud_core_course_renderer extends core_course_renderer {
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
        global $CFG, $OUTPUT;
        $type = theme_ceajud_get_setting('frontpagerenderer');
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course, $type=3) {
        ...
        $coursecontacts = theme_ceajud_get_setting('tilesshowcontacts');
        if ($coursecontacts) {
            $coursecontacttitle = theme_ceajud_get_setting('tilescontactstitle');
